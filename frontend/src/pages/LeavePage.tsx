...
export function LeavePage() {
  ...
  async function load(selectedFilter = filter) { try { setError(''); setRequests(await getLeave(selectedFilter === 'all' ? undefined : selectedFilter)) } catch (requestError) { setError(apiErrorMessage(requestError)) } finally { setLoading(false) } }
  useEffect(() => { getLeave(filter === 'all' ? undefined : filter).then(setRequests).catch((requestError) => setError(apiErrorMessage(requestError))).finally(() => setLoading(false)) }, [filter])
  async function submit(event: React.FormEvent) { event.preventDefault(); setError(''); setSuccess(''); if (!form.startDate || !form.endDate || form.startDate > form.endDate) { setError('Choose valid leave dates, with the end date on or after the start date.'); return } if (!form.reason.trim()) { setError('Add a reason for your leave request.'); return } try { setSaving(true); await createLeave({ ...form, reason: form.reason.trim() }); setForm({ type: 'paid', startDate: '', endDate: '', reason: '' }); setOpen(false); setSuccess('Leave request submitted for approval.'); await load() } catch (requestError) { setError(apiErrorMessage(requestError)) } finally { setSaving(false) } }
  return <>
    ...
    {error && <ErrorState message={error} />}{success && <div className="success-message" role="status">{success}</div>}
    ...
    <div className="leave-filter" role="group" aria-label="Leave request status">{filters.map((item) => <button className={filter === item.value ? 'selected' : ''} key={item.value} onClick={() => { setLoading(true); setFilter(item.value) }} type="button">{item.label}</button>)}</div>
